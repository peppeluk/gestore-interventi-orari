...
function bindEvents() {
  ...
  const sheetsBtn = document.getElementById("menuSheetsBtn");
  if (sheetsBtn) {
...
function renderClientsDatalist() {
  ...
  state.clients.filter(c => !c.deleted).forEach(client => {
...
function renderClientsList() {
  ...
  let clients = state.clients.filter(c => !c.deleted);
...
function deleteClient(clientId) {
  ...
  const linkedCount = state.interventions.filter(i => i.clientId === clientId && !i.deleted).length;
  if (linkedCount > 0) {
...
function getClientById(clientId) {
  return state.clients.find(c => c.id === clientId && !c.deleted) || null;
}

function getClientInterventions(clientId) {
  return state.interventions.filter(i => i.clientId === clientId && !i.deleted);
}
...
function filterDeletedRecords() {
  state.clients = state.clients.filter(c => !c.deleted);
  state.interventions = state.interventions.filter(i => !i.deleted);
}
...
function saveState() {
  ...
  if (typeof triggerSheetsPush === "function") {
    triggerSheetsPush();
  }
}
